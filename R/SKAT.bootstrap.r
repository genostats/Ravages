SKAT.bootstrap <- function(x, group = x@ped$pheno, genomic.region = x@snps$genomic.region, 
                 Pi, X, weights = (1-x@snps$maf)**24, maf.threshold = 0.5, 
  ...
  # matrice des proba d'appartenir au groupe
  if(missing(Pi)) {
