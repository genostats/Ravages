run.skat <- function(x) {
  ...
  pool <- Ravages:::SKAT(x, ifelse(x@ped$pheno == 0, 0, 1), weights = SKAT:::Beta.Weights(x@snps$maf, c(1,25)), maf.threshold = maf)

  x.1 <- select.inds(x, pheno != 2)
  x.2 <- select.inds(x, pheno != 1)
