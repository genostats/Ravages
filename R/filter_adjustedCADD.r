filter.adjustedCADD <- function(x, variant.scores = NULL, ref.level, filter = c("whole", "controls", "any"), maf.threshold = 0.01, min.nb.snps = 2, min.cumulative.maf, group, cores = 10, verbose = T) {
  ...
  if(!("adjCADD" %in% colnames(x@snps))){  
    ...
    if(is.null(variant.scores)){
      ...
      x@snps <- merge(x@snps, x.scores, by = c("chr", "pos", "A1", "A2"), sort.x = F, all.x = T)
    }else{
      ...
      if(verbose) cat("Annotation of variants with provided scores in 'variant.scores'. Warning: these scores should correspond to the adjusted CADD scores available at https://lysine.univ-brest.fr/CADD_Regions/AdjustedCADD_v1.4_202108.tsv.gz\n")
      x@snps <- merge(x@snps, variant.scores, by = c("chr", "pos", "A1", "A2"), sort.x = F, all.x = T)
    } 
  }
  else{
