...
Eigen::MatrixXd GPG(XPtr<matrix4> pA, LogicalVector which_snps, NumericVector p, NumericVector weights, 
  ...
  return GPG(data, pA->true_ncol, pp, W, P, N, symmetrize);
}
...
//  Rcpp::Rcout << "gpes = " << k << "\n";
  ...
  for(int i = 0; i < k; i++) {
    for(int j = i; j < k; j++) {
      ...
      NumericMatrix tmp1 = WLP2(&data[0], &pp[0], m, n, true_ncol, W, tmp0);
      NumericMatrix tmp2 = WLP2(&data[0], &pp[0], m, n, true_ncol, W, tmp1);
...
BEGIN_RCPP
    ...
    rcpp_result_gen = Rcpp::wrap(GPG(pA, which_snps, p, weights, P, N, true));
