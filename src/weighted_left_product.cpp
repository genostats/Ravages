...
struct paraWLP : public Worker {
  ...
  //constructeur
  paraWLP(const uint8_t ** data, size_t nrow, size_t ncol, size_t true_ncol, std::vector<double> we, size_t r, double * v, double * Av)
          : data(data), nrow(nrow), ncol(ncol), true_ncol(true_ncol), we(we), r(r), v(v), Av(Av) { }
...
NumericMatrix WLP(XPtr<matrix4> pA, const std::vector<double> & we, NumericMatrix & v) {
  return WLP(const_cast<const uint8_t **>(pA->data), pA->nrow, pA->ncol, pA->true_ncol, we, v);
}
...
NumericMatrix WLP(const uint8_t ** A_data, size_t A_nrow, size_t A_ncol, size_t A_true_ncol, const std::vector<double> & we, NumericMatrix & v) {
  ...
  NumericMatrix R(n,r);
  paraWLP X(A_data, A_nrow, A_ncol, A_true_ncol, we, r, v.begin(), R.begin());
