...
SKAT(x1, group=x1@ped$pop, perm.max = 500)
...
To compute the p-values, a chi-square approximation is used based on the statistics' moments. The moments can either be estimated using a sampling procedure,or be analytically computed using the method from Liu et al. 2008.  
If the sample size is lower than 2000, we recommand to use the sampling procedure. If no covariates are present, a simple permutation procedure can be used (*get.moments = "permutations"*), otherwise, a boostrap procedure is used (*get.moments = "bootstrap"*). For those two situations, a sequential is used to compute the p-values: permutated statistics are computed and each one is compared to the observed statistics. The sampling procedure stops when either *perm.target* (the number of times a permutated statistics should be greater than the observed statistics) or *perm.max* is reached (the maximum number of permutations to perform). P-values are then computed in two different ways: if *perm.target* is reached, the p-value is computed as *perm.target* divided by the number of permutations performed to reach this value; if *perm.max* is reached before *perm.target* (that is, for pretty small p-values), p-values are computed using the chi-square approximation based on moments estimated from the permutated statistics. *perm.target* and *perm.max* should be given as a list to the argument *params.sampling* of SKAT.  
If the sample size is bigger than 2000, the analytical calculation from Liu et al. can be used to compute the theoretical moments.  
It is possible to clearly ask for a specific method to compute the moments using *get.moments = "permutations"*, *"bootstrap"* or *"theoretical"*. If *get.moments = "size.based"*, the method will be choosed based on the sample size. For each one of these methods, the chi-square approximation can be based on the first thre moments (*estimation.pvalue = "skewness"*), or on moments 1, 2 and 4 to have a more precise estimation (*estimation.pvalue = "kurtosis"*). If *get.moments = "theoretical"* and *estimation.pvalue = "skewness"*, it is equivalent to the "liu" method in the SKAT package, and if *estimation.pvalue = "kurtosis"*, it corresponds to the "liu.mod" methid in the SKAT package. If *debug = TRUE*, the statistics' moments will be returned in addition to the p-values.   
If the theoretical method from Liu et al. are used, it is possible to parallelise the calculations using the argument *cores* set at 10, by default.
