...
#Null model
skat.null.3gps <- NullObject.parameters(x.filter@ped$pheno, 
...
x.filter@ped$pheno.pooled <- factor(ifelse(x.filter@ped$pheno == 0, 0, 1))
skat.null.2gps <- NullObject.parameters(x.filter@ped$pheno.pooled, 
                                        RVAT = "SKAT", pheno.type = "categorial")
...
#Null model
H0.cas1 <- NullObject.parameters(x.1@ped$pheno, ref.level = 0, 
                                 RVAT = "burden", pheno.type = "categorial")
H0.cas2 <- NullObject.parameters(x.2@ped$pheno, ref.level = 0, 
                                 RVAT = "burden", pheno.type = "categorial")
#Run WSS
cast.cas1 <- burden(x.1, H0.cas1, burden="CAST", cores = 1)
cast.cas2 <- burden(x.2, H0.cas2, burden="CAST", cores = 1)
mean(cast.cas1$p.value<0.05) ; mean(cast.cas2$p.value<0.05)
```
