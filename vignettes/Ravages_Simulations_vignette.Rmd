...
## Power calculation
Power of the RVAT can be directly computed using **Ravages**. To do so, the functions *random.bed.matrix.power()* and *rbm.haplos.power()* can be used which rely on simulations based on GRR and on haplotypes respectively. Those two functions use the same arguments as the simulation functions previously presented. Power calculations are available for *CAST*, *WSS* and *SKAT*. In addition, using *random.bed.matrix.power()*, theoretical power can be rapidly computed for CAST by using **power.type="theoretical"** and **RVAT="CAST"**. Below are an example of CAST power based on theoretical calculations or simulations with haplotypes. 
```{r}
#CAST theoretical power
GRR.del <- GRR.matrix(GRR = "SKAT", genes.maf = Kryukov, n.case.groups = 2,
                      GRR.multiplicative.factor=2, select.gene = "R1")
random.bed.matrix.power(genes.maf = Kryukov, size = c(1000, 500, 500),
                        prev = c(0.001, 0.001), GRR.matrix.del = GRR.del,
                        p.causal = 0.5, p.protect = 0, select.gene="R1",
                        same.variant = FALSE, genetic.model = "multiplicative",
                        power.type="theoretical")

#WSS power based on simulations on haplotypes
