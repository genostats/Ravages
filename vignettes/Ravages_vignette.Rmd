...
To run the SKAT association test, the function **SKAT.NullObject()** first need to be called. The group of each individual should be given as a factor to *group*, and potential covariates should be included as a data.frame to *data*. If only some of them are to be include, the should be given as a R formula to *formula*. This function will compute parameters to use the **SKAT()** function, and should be given to this function using the argument *NullObject*.  
...
x1.null.covar <- SKAT.NullObject(x1@ped$pop, data = covar)
