...
SKAT(x1, x1.H0.SKAT, params.sampling=list(perm.target = 100, perm.max = 5e4))
...
x1.H0 <- NullObject.parameters(x1@ped$pop, ref.level = "CEU", 
                               RVAT = "burden", pheno.type = "categorial")
# WSS 
burden.mlogit(x = x1, NullObject = x1.H0, burden ="WSS", 
...
burden.mlogit(x = x1, NullObject = x1.H0.covar, burden = "WSS")

#WSS using directly the score matrix computed previously
burden.mlogit(NullObject = x1.H0.covar, burden=WSS.score)
```
...
#Null model
x1.H0.continuous <- NullObject.parameters(pheno1, RVAT = "burden", pheno.type = "continuous")
#Test CAST 
burden(x1, NullObject = x1.H0.continuous, burden = "CAST")
#Test WSS with covariates
#Null model
x1.H0.continuous.covar <- NullObject.parameters(pheno1, RVAT = "burden", 
                                                pheno.type = "continuous",
                                                data = covar, formula = ~ sex)
burden(x1, NullObject = x1.H0.continuous.covar, burden = "WSS")
```
...
Before running the SKAT association test, the function **NullObject.parameters()** first need to be called as for the burden tests, by specifying *RVAT = "SKAT"*. The group of each individual should be given as a factor to *pheno* if a categorail phenotype is studied, and as a numeric vector is a continuous phenotype is studied. As before, potential covariates should be included as a data.frame to *data*. If only some of them are to be include, they should be given as a R formula to *formula*. This function will compute parameters to use the **SKAT()** function, and should be given to this function using the argument *NullObject*.  
...
#Bootstrap: include covariates
x1.null.covar <- NullObject.parameters(x1@ped$pop, RVAT = "SKAT", 
                                       pheno.type = "categorial", data = covar)
SKAT(x1, x1.null.covar, get.moments = "bootstrap", debug = TRUE,
     params.sampling = list(perm.target = 100, perm.max =5e4))
