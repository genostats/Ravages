...
**Ravages** also offers the possibility to perform genetic simulations based on real data haplotypes to mimic linkage desequlibrium pattern observed on these data.  
...
For the second situation, **rbm.haplos.thresholds()** requires a matrix of haplotypes (argument *haplos* as well), a matrix containing the burden of each haplotype (*burdens* with one row per haplotype), the standard deviation associated to non-genetic elements (*sd = sqrt(1-h2)*, *h2* being the phenotypic variance explained by the gene), and the threshold used to compute haplotypes probabilities in each group of individuals (*thr1*). This threshold is chosen from a liability distribution on adjusted burdens and each haplotype probability in each group of individuals is computed according to the fact that liability in this group is between *thr1* and +Inf. Pairs of haplotypes are then sampled for each individual according to these previous conditionnal probabilities. 
The arguments *burdens*, *sd*, and *thr1* can be obtained using the function **get.rbm.thresholds()**. This function requires as the other ones the argument *haplos*, and a number of other arguments to draw the libaility model. *weights* is a vector whose length corresponds to the number of variants and is used to weight variant into burdens computation (by default, the same formula as in SKAT is used, which gives the higher weights to the rarest variants : $w=-0.4*|log_{10}(MAF)|$). *maf.threshold* corresponds to the maf threshold used to define a rare variants, i.e. all monomorphic variants and variants with a MAF upper than this threshold will have a weight of 0. In addition, the number of causal variants needs to be given to *nb.causal*, and the proportion of protective variants among causal variants needs to be given to *p.protect*. Haplotypes' burdens will be computed on these causal variants. Finally, *h2* corresponds to the phenotypic variant explained by the gene: haplotypes burdens will be adjusted on this value; and *prev* corresponds to the prevalence of each group of individuals: *thr1* will correspond to the quantile associated to 1-*prev*. This quantile is drawn from a standard normal distribution if *normal.approx*=TRUE (applicable for low *h2* value and pretty large *prev* values), otherwise, quantiles will be drawn from a distribution based on observed sampled burdens. If one wants to simulate a group of unselected controls, *prev* needs to be set at 1, regardless of the other arguments: haplotypes are sampled uniformly in the whole distribution.  
*nb.causal*, *p.protect*, *h2* and *prev* should be vector of equal size as the number of groups. If they correspond to a single value, this value will be repeated.  
As for the genetic simulations based on GRR, when using **rbm.haplos.freqs()** or **rbm.haplos.thresholds()**, the sizes of each group should be given to *size*, and the number of simulations to *replicates*. As before, the replicate number will be in the slot *@snps$genomic.region* and the group of individuals in *@ped$pheno*. If **rbm.haplos.freqs()** is used, phenotype values will correspond to the colnames of *freqs*.  
...
#Simulations based on the liability model
#Parameters for simulations with the second group of cases
#having half h2 and twice the prevalence compared to the first one, 30 causal variants
x.param <- get.rbm.thresholds(haplos=LCT.gene.hap, maf.threshold = 0.01, nb.causal=30,
...
x.GRR.3 <- select.inds(x.GRR.filter, x.GRR.filter@ped$pheno %in% c(0,3))
wss.cas1 <- burden.mlogit(x.GRR.1, burden="WSS", ref.level=0)
wss.cas2 <- burden.mlogit(x.GRR.2, burden="WSS", ref.level=0)
wss.cas3 <- burden.mlogit(x.GRR.3, burden="WSS", ref.level=0)
mean(wss.cas1<0.05) ; mean(wss.cas2<0.05) ; mean(wss.cas3<0.05)



#Simulations based on haplotypes and the liability model
#One group of controls and two groups of cases genetically heterogeneous 
x <- rbm.haplos.thresholds(haplos=LCT.gene.hap, burdens=x.param$burdens, sd=x.param$sd, 
                           thr1=x.param$thr1, size=c(200, 100, 100), replicates=100)
#Filtering of rare variants: MAF<1% in the entire sample
x.filter <- filter.rare.variants(x, maf.threshold = 0.01, filter="whole")

#Power of SKAT using the multinomial regression: each group is
#considered separately; significance level=5%
skat <- SKAT(x.filter)$p.value
mean(skat<0.05)
#Considering all the cases as one group and compare them to the controls
pheno.pooled <- ifelse(x.filter@ped$pheno == 0, 0, 1)
skat.pooled <- SKAT(x.filter, group=pheno.pooled)$p.value
mean(skat.pooled<0.05)

#Power of the burden test CAST comparing each group of cases to the controls
#at a 5% significance threshold
x.1 <- select.inds(x.filter, x.filter@ped$pheno != 2)
x.2 <- select.inds(x.filter, x.filter@ped$pheno != 1)
cast.cas1 <- burden.mlogit(x.1, burden="CAST", ref.level=0)$p.value
cast.cas2 <- burden.mlogit(x.2, burden="CAST", ref.level=0)$p.value
mean(cast.cas1<0.05) ; mean(cast.cas2<0.05)
```
