...
#Causal variants are sampled twice
x <- rbm.haplos.thresholds(haplos=LCT.gene.hap, nb.causal = 30, h2=c(0.01,0.01,0.02),
                           prev=c(1,0.01,0.005), size=c(200, 100, 100), replicates=100,
                           rep.by.causal = 50)
...
#Null model
skat.null.3gps <- SKAT.NullObject(factor(x.filter@ped$pheno))
skat.3gps <- SKAT(x.filter, skat.null.3gps, 
...
x.filter@ped$pheno.pooled <- ifelse(x.filter@ped$pheno == 0, 0, 1)
skat.null.2gps <- SKAT.NullObject(factor(x.filter@ped$pheno.pooled))
skat.2gps <- SKAT(x.filter, skat.null.2gps, 
                  params.sampling = list(perm.target = 100, perm.max = 1000))
mean(skat.2gps$p.value<0.05)
