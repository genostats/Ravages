...
* the same GRR is given to all the variants (*GRR="constant"*), its value being specified to the argument *GRR.value*;  
* the GRR is computed using the formula from the publication presenting the method SKAT (*GRR="SKAT"*);   
* the GRR is computed using another function depending on MAF in the general population (*GRR="variable"*), this function being specified to the argument *GRR.function*.  
...
Below is an exemple of data simulation with the scenario DG:
```{r}
...
Below is an exemple of data simulation with the scenario DVSG and power calculation:
